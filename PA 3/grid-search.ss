...
(define move-robot
  (lambda (grid x y count)
    (let ((dir (random 4)))
      (cond
        ((and (= dir n) (> x 0) (< (get-node grid (- x 1) y) obstacle) (not (= (get-node grid (- x 1) y) visited)))
           (set! robot (list (- x 1) y)))
        ((and (= dir s) (< x (- num-col-row 1)) (< (get-node grid (+ x 1) y) obstacle) (not (= (get-node grid (+ x 1) y) visited)))
           (set! robot (list (+ x 1) y)))
        ((and (= dir w) (> y 0) (< (get-node grid x (- y 1)) obstacle) (not (= (get-node grid x (- y 1)) visited)))
           (set! robot (list x (- y 1))))
        ((and (= dir e) (< y (- num-col-row 1)) (< (get-node grid x (+ y 1)) obstacle) (not (= (get-node grid x (+ y 1)) visited)))
           (set! robot (list x (+ y 1))))
        ((> count 100)
